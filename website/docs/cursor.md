...
Override the projects root with `CURSOR_PROJECTS_DIR`. Lines look like `{ "role", "message": { "content": [ { "type": "text"|"tool_use"|"tool_result"|"thinking", ... } ] } }`. Roles are `user`, `assistant`, and `system`. Subagent transcripts are skipped.
