...
Each line is a JSON object. Messages have a `type` field (`user` or `assistant`) and a nested `message.content` array. Extract keeps **text**, **thinking**, **tool_use**, and **tool_result** parts in the IR. Inject writes them back the same way.
