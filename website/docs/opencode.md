...
OpenCode uses a normalized SQLite schema with `session`, `message`, and `part` tables. Extract maps `text`, `reasoning`, and `tool` parts into the IR. Message `role` may be `user`, `assistant`, or `system`. Inject writes those part types back, and skips tool calls that have no matching tool result (Cursor JSONL often records `tool_use` without `tool_result` in the same file).
