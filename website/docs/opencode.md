...
OpenCode uses a normalized SQLite schema with `session`, `message`, and `part` tables. Extract maps `text`, `reasoning`, and `tool` parts into the IR. Message `role` may be `user`, `assistant`, or `system`. Inject writes those part types back.
